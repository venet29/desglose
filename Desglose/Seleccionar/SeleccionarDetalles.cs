...
{
    public class SeleccionarDetalles
    {
        private readonly UIApplication uiapp;
        private Document _doc;
        private UIDocument _uidoc;

        public SeleccionarDetalles(UIApplication _uiapp)
        {
            uiapp = _uiapp;
        ...
        public bool SeleccionarElevaciones()
        {
            try
            {
                ISelectionFilter f = new DetallesElevacionVigaFilter();
                //selecciona un objeto floor
                var _ListaRebarSeleccionado = _uidoc.Selection.PickElementsByRectangle(f, "Seleccionar barra (Rebar)").Select(c=>c.Id).ToList();
